...
class DecoderLayer(nn.Module):
    def __init__(self, embed_dim, side_info_dim, side_info_embed_dim, dropout, attention_method, n_heads, n_channels=1024, att_type='q'):
        ...
        self.dec_self_attn_side_info_enc = nn.Linear(side_info_dim, 20).to(device)
        tot_dim = embed_dim + side_info_embed_dim
        
        
        if self.att_type == 'q':
            self.dec_enc_attn_side_info_enc = nn.Linear(side_info_dim, 20).to(device)            

            self.forward = self.forward_q
        else:
            self.dec_enc_attn_side_info_enc = nn.Linear(side_info_dim, 49).to(device)
    ...
    def forward_q(self, dec_inputs, enc_outputs, side_info, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        """
        self_attn_side_info = self.dec_self_attn_side_info_enc(side_info).unsqueeze(2).repeat(1,1, dec_inputs.size(2))

        self_att_q = dec_inputs * self_attn_side_info

        dec_outputs, dec_self_attn = self.dec_self_attn(self_att_q, dec_inputs,  dec_inputs, dec_self_attn_mask)

        cross_attn_side_info = self.dec_enc_attn_side_info_enc(side_info).unsqueeze(2).repeat(1, 1, dec_outputs.size(2))
        cross_att_q = dec_outputs * cross_attn_side_info
...
#             cross_att_q = cross_att_q.permute(0, 2, 1).contiguous()
    ...
    def forward_k(self, dec_inputs, enc_outputs, side_info, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        """
        self_attn_side_info = self.dec_self_attn_side_info_enc(side_info).unsqueeze(2).repeat(1, 1, dec_inputs.size(2))
        self_att_k = dec_inputs * self_attn_side_info

        dec_outputs, dec_self_attn = self.dec_self_attn(dec_inputs, self_att_k,  dec_inputs, dec_self_attn_mask)
        cross_attn_side_info = self.dec_enc_attn_side_info_enc(side_info).unsqueeze(2).repeat(1, 1, enc_outputs.size(2))
        cross_att_k = enc_outputs * cross_attn_side_info
