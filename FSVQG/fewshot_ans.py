        ...
        if args.mode == 'Train':
            ...
            if epoch % 30 == 0:
                torch.save(vqg_net.state_dict(),
                            ...
                            'epoch{}_{}_{}_{}_new{}shot_non_meta_ans.pkl'.format(epoch, args.network, args.dataset_type, lm_encoder, args.train_query)))

                torch.save(img_encoder.state_dict(),
                            os.path.join(args.model_path,
                            'img_encoder/epoch{}_{}_{}_{}_new{}shot_non_meta_ans.pkl'.format(epoch, args.network, args.dataset_type, lm_encoder, args.train_query)))
...
#         img_train = imgs
        ...
        preds, alphas = model(features2, ans, alengths, captions)
