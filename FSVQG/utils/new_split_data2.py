...
def save_dataset(image_dir1, image_dir2, questions, annotations, vocab, ans2cat, output,
    ...
    ans_vocab = load_vocab("data/processed/vocab_iq_ans.json")
    # print(len(vocab))
...
#        "images", (total_images, im_size, im_size, 3), dtype='f')
    ...
    d_qids = h5file.create_dataset(
        "question_ids", (total_questions,), dtype='i')
...
#             image = transform(image).unsqueeze(0).cuda()
            ...
            # print()
            # exit()
#             print(encoder.extract_features(image).squeeze().size())
#             exit()
            d_images[i_index, :, :] = np.array(image)
#             print(encoder(image).squeeze().cpu().detach().numpy().shape)
...
#         # print(q)
        ...
        a, length = process_text(answer, ans_vocab,
