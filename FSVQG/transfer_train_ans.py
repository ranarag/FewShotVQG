    ...
    for epoch in range(start_epoch + 1, args.epochs):
        if args.mode == 'Train':
            ...
            torch.save(vqg_net.state_dict(),
                        ...
                        'epoch{}_{}_{}_{}_{}_new{}shot_transfer_ans.pkl'.format(epoch, args.network, args.metalr_type, args.dataset_type, \
                                                                       lm_encoder, args.train_query)))

#         Uncomment for meta-testing after each epoch
        if args.mode == 'Test':
            
            score = test(nlge, vqg_net, cross_entropy_loss, valset, word_dict, ans_dict, args, None)
...
#             print(imgs.size())
        ...
        preds, alphas = model(imgs, ans, alengths, cap)
...
def test(nlge, maml, cross_entropy_loss, valset, word_dict, ans_dict, args, log_interval):
    K = args.way
    N = args.train_query
    p = K*N
    ...
    if not args.scaling_shifting:
        model_name += "_no_SS"
    os.makedirs('{}_{}_transfer_ans_res{}shot'.format(model_name, args.dataset_type, N), exist_ok=True)
    print(num_iter)
        ...
        for idx,(imgs, captions, answer, cat, qids, img_ids) in enumerate(tqdm_gen):
            ...
            tqdm_gen.set_description('RUN {},'
                                     ...
                                     ' CIDEr={:.4f}'.format(epoch, 
                                                           ...
                                                           cider.avg))
        with open('{}_{}_transfer_ans_res{}shot/preds_gts_dict_run{}.json'.format(model_name, args.dataset_type, N, epoch), 'w') as fid:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval-every', type=int, default=1, metavar='E',
                        help='number of epochs to train for (default: 10)')
