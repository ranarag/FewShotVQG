...
def main(args):
    ...
    if args.mode == 'Train':
        if args.dataset_type == '7w':
            ...
            train_sampler = UnsupSampler7w(args.qid2data, 4)
    ...
    if len(args.model):
        ...
        vqg_net = vqg_net.cuda()
        img_encoder = img_encoder.cuda()
#         vqg_net = nn.DataParallel(vqg_net).cuda()
#         img_encoder = nn.DataParallel(img_encoder).cuda()
        ...
        if args.mode == 'Train':
            ...
            if epoch % 1 == 0:
                torch.save(vqg_net.state_dict(),
                            ...
                            'epoch{}_{}_{}_{}_new{}shot_non_meta_cats.pkl'.format(epoch, args.network, args.dataset_type,lm_encoder, args.train_query)))

                torch.save(img_encoder.state_dict(),
                            os.path.join(args.model_path,
                            'img_encoder/epoch{}_{}_{}_{}_new{}shot_non_meta_cats.pkl'.format(epoch, args.network,args.dataset_type, lm_encoder, args.train_query)))
...
#         img_train = imgs
        ...
        preds, alphas = model(features2, cat, captions)
...
#             with torch.no_grad():
#                 if args.network == 'effnet':
#                     imgs = img_encoder.extract_features(imgs).detach()
#                 else:
#                     imgs = img_encoder(imgs).detach()
...
#             img_ids_test = img_ids[p:]
            ...
            n_img_encoder = n_img_encoder.cuda()
...
#                     print(ans_test[0])
            ...
            tqdm_gen.set_description('RUN {},'
                                     ...
                                     ' CIDEr={:.4f}'.format(epoch, 
                                                           ...
                                                           cider.avg))
        with open('{}_{}_{}_cat_res{}shot/preds_gts_dict_run{}.json'.format(model_name, args.dataset_type, lm_encoder, N, epoch), 'w') as fid:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cat-model', type=str,
                        default='',
                        help='path to model')
    parser.add_argument('--qid2indx', type=str,
                        default='',
                        help='path to model')
