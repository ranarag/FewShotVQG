...
#     with torch.no_grad():
#         category_embeds, cat_biases = learner.module.category_encoder(cat_test)
#         recasted_embeds, recasted_biases = recast_category_embeds(category_embeds, cat_biases, img_test.size(2))
#         n_img_features = img_test.mul(recasted_embeds) + recasted_biases
...
#         scheduler.step()
        print('EPOCH {},'
              ...
              ' CIDEr={:.4f}'.format(epoch,
                                     ...
                                     cider.avg))
        # histories.sendLoss(loss=loss, acc=acc1, epoch=epoch, total_epochs=args.epochs)
#         torch.save(vqg_net.decoder.state_dict(),
#                     os.path.join(args.model_path, 'meta/','nbn_decoder/',
#                     'Decoder-epoch-%d.pkl'% (epoch+1)))
#         torch.save(vqg_net.category_encoder.state_dict(),
#                     os.path.join(args.model_path, 'meta/','nbn_category_encoder/',
#                     'cat-encoder-epoch-%d.pkl'% (epoch+1)))
...
def meta_train(runs, nlge, maml, optimizer, cross_entropy_loss,
    ...
    for epoch in tqdm_gen:
        ...
        optimizer.zero_grad()
        for  (imgs, captions, _,  _, cat, _, qid) in train_loader:
...
#         cat_qid_dict_list = []
        ...
        for (imgs, captions, _,  _, cat, _) in tqdm_gen:

            
            
            imgs = imgs.cuda().squeeze()
#             unique_cat = torch.unique(cat)
#             print(unique_cat.numpy())
#             exit()
#             cat_qid_dict = {"cat": [int(x) for x in unique_cat.numpy().tolist()], "question_ids": [int(x) for x in qids]}
#             cat_qid_dict_list.append(cat_qid_dict)
#             continue
...
#                 print(best_score)
            ...
            tqdm_gen.set_description('RUN {},'
                                     ...
                                     ' CIDEr={:.4f}'.format(epoch, 
                                                            ...
                                                            meteor.avg, rouge_l.avg,
                                                           cider.avg))
#         with open('cats_qids_for_testing_run{}.json'.format(epotui ektu amar gpu0 er space ich+1), 'w') as fid:
#             fid.write(json.dumps(cat_qid_dict_list))
#     print('Bleu 1={:.4f} Bleu 2={:.4f}'
#           ' Bleu 3={:.4f} Bleu 4={:.4f}'
#           ' METEOR={:.4f} ROUGE_L={:.4f}'
#           ' CIDEr={:.4f}'.format(bleu_1.avg,
#                                          bleu_2.avg,
#                                          bleu_3.avg,
#                                          bleu_4.avg,
#                                          meteor.avg,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num-batch', type=int, default=32, metavar='N',
                        help='batch size for training (default: 6tui ektu amar gpu0 er space i4)')
    ...
    parser.add_argument('--cat-model', type=str, help='path to model')#, default='model/meta/nbn_category_encoder/best_fxmatch_bert_ft_new10shot.pkl')
    parser.add_argument('--decoder-model', type=str, help='path to model')#, default='model/meta/nbn_decoder/best_fxmatch_bert_ft_new10shot.pkl')
    ...
    parser.add_argument('--val-dataset', type=str,
                        default='data/processed/latest_val_fxmatch_img_iq_dataset.hdf5',
                        help='Path for train annotation json file.')
    ...
    parser.add_argument('--train_query', type=int, default=1) # (Shot) The number of meta train samples for each class in a task
