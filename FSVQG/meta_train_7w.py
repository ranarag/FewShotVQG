...
from utils import datasets, samplers, newcollate_fn
...
#                                         word_dict)
#     print("DONE fetching")
    with open('bert_embedding7w.pkl', 'rb') as fid:
...
#     embedding=None
    ...
    trainset = datasets.get_class_obj('Val', 'img_id2embeds_effnet_7w.hdf5',
                            'qid2toks7w_train.pkl',
                             transform=new_transform, 
                             max_examples=None)
    
    train_sampler = samplers.get_class_obj('Visual7w', 'train_cat2qid7w.json',
                                      1000,
                                      args.way,
                                      args.train_query,
                                      args.test_query)
    
    train_loader = datasets.get_class_obj('Val', 'img_id2embeds_effnet_7w.hdf5',
                            'qid2toks7w_test.pkl',
                             transform=new_transform, 
                             max_examples=None)
...
#             torch.save(vqg_net.decoder.state_dict(),
#                         os.path.join(args.model_path, 'meta/','nbn_decoder/',
#                         'best_effnet7w_bert_new10shot.pkl'))
#             torch.save(vqg_net.category_encoder.state_dict(),
#                         os.path.join(args.model_path, 'meta/','nbn_category_encoder/',
#                         'best_effnet7w_bert_new10shot.pkl'))
        #Uncomment for meta-testing after each epoch
...
def meta_train(runs, nlge, maml, optimizer, cross_entropy_loss,
    ...
    for epoch in tqdm_gen:
        ...
        optimizer.zero_grad()
        for  (imgs, captions, answers, cat, qids, _) in train_loader:
...
#         cat_qid_dict_list = []
        ...
        for (imgs, questions, aptions, cat, qids) in tqdm_gen:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cat-model', type=str, help='path to model', default='model/meta/nbn_category_encoder/best_effnet7w_bert_new10shot.pkl')
    parser.add_argument('--decoder-model', type=str, help='path to model', default='model/meta/nbn_decoder/best_effnet7w_bert_new10shot.pkl')
    ...
    parser.add_argument('--vocab-path', type=str,
                        default='data/processed/vocab_7w.json',
                        help='Path for vocabulary wrapper.')
    parser.add_argument('--dataset', type=str,
                        default='latest_train_imgnet_iq_dataset.hdf5',
